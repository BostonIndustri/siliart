...
(function (factory) {
	 ...
	 */
	PsMessages.prototype.new_message = function (user_id, flag, btn, opts) {
		// open chat
		if (window.ps_chat && peepso.screenSize() === 'large' && btn) {
			...
			peepso.postJson('chatajax.get_chat_with', { recipient: user_id }, function (response) {
				if (response.success) {
					ps_chat.open_chat(response.data.msg_id, opts);
				}
				setTimeout(function () {
					loading.hide();
				}, 1000);
		...
		observer.doAction('new_message_dialog', elem, {
			postbox: $message_postbox,
			opts
		});

		observer.addAction('new_message_dialog_remove_attachment', function ($elem) {
			opts = null;
		});
				...
				if (window.ReactNativeWebView !== undefined) {
					obj.popover_list.find(selector).on('click', function () {
						window.ReactNativeWebView.postMessage(
							JSON.stringify({
								clickedLink: $(this).data('url'),
								preferredTab: 'messages'
							})
						);
					});
				} else if (window.ps_chat && peepso.screenSize() === 'large') {
					obj.popover_list.find(selector).on('click', function () {
