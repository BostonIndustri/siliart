<?php /**/
...
class PeepSoPhotosAdmin
{
    ...
    private function __construct()
    {
        ...
        add_filter('peepso_report_column_content_summary', array(&$this, 'report_column_content_summary'));
    ...
    public function report_column_title($title, $item, $column_name)
    {
        if ('post_title' === $column_name) {
            ...
            if (PeepSoSharePhotos::MODULE_ID === intval($item['rep_module_id'])) {
                ...
                $photo = $photos_model->get_photo($item['rep_external_id']);
    ...
    public function report_column_content_summary($item) {
        if (PeepSoSharePhotos::MODULE_ID === intval($item['rep_module_id'])) {
            $photos = PeepSoSharePhotos::get_instance();
            return (array) $photos->get_photos_model()->get_photo($item['rep_external_id']);
        }
        return $item;
    }
