<?php /**/
...
//			}
	...
	private function get_amazon_s3()
	{
		...
		$aws_s3_region = PeepSo::get_option('photos_aws_bucket_location', 'us-east-1');
		...
		try {
			$s3 = new PeepSoPhotosS3($aws_access_key_id, $aws_secret_access_key);
			$s3->setRegion($aws_s3_region);

			$buckets = $s3->listBuckets();
			if (!in_array($aws_s3_bucket, $buckets)) {
                throw new Exception('Bucket ' . $aws_s3_bucket . ' is not exists.');
			}
		} catch (PeepSoPhotosS3Exception $e) {
	...
	public function upload_to_amazon_s3($filepath)
	{
		...
		$result = $s3->putObject(file_get_contents($filepath), $aws_s3_bucket, $filename, $s3::ACL_PUBLIC_READ, [], ['Content-Type' => $check['type']]);

		if ($result) {
			return $result;
		} else {
			new PeepSoError('[PHOTOS] ' . __('There was a problem when uploading the file', 'picso'));
		}
	}
	...
	public function remove_from_amazon_s3($files)
	{
		...
		try {
			...
			foreach ($files as $file) {
				$url = parse_url($file);
				$filename = substr($url['path'], 1);

				$result = $s3->deleteObject($aws_s3_bucket, $filename);
				if (!$result) {
					new PeepSoError('[PHOTOS] '.__('There was a problem when deleting the file', 'picso'));
				}
			}
	...
	public function delete_photo($photo)
	{
		...
		PeepSoSharePhotos::delete_widget_mayfly($post->post_author);
	...
	public function delete_photo_comment($activity)
	{
		...
		PeepSoSharePhotos::delete_widget_mayfly($activity->act_owner_id);
...
//		return ($image); // no need to return, passed in by reference
	...
	public function imagick_strip($file) {
        ...
        if(!apply_filters('peepso_filter_photos_imagick_strip', TRUE)) return;
