...
class BP_REIGN_Members_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$name        = esc_html_x( 'Reign - Members Carousel', 'widget name', 'buddypress' );
					...
					while ( bp_members() ) :
						...
						<div class="rg-member rg-image-box">
							...
							<div class="item-avatar">
								<a href="<?php bp_member_permalink(); ?>"><?php echo reign_get_online_status( $user_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><?php echo get_avatar( bp_get_member_user_id(), 100 ); ?></a>
							</div>
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['display_member_name'] = isset( $new_instance['display_member_name'] ) ? wp_strip_all_tags( $new_instance['display_member_name'] ) : 'show';
	...
	public function form( $instance ) {
		...
		<p>
			...
			if ( 'newest' === $member_default ) :
				?>
					selected="selected"<?php endif; ?>><?php esc_html_e( 'Newest', 'buddypress' ); ?></option>
				<option value="active"
				<?php
				if ( 'active' === $member_default ) :
					?>
					selected="selected"<?php endif; ?>><?php esc_html_e( 'Active', 'buddypress' ); ?></option>
				<option value="popular"
					<?php
					if ( 'popular' === $member_default ) :
						?>
					selected="selected"<?php endif; ?>><?php esc_html_e( 'Popular', 'buddypress' ); ?></option>
			</select>
		</p>
			...
			<select name="<?php echo esc_attr( $this->get_field_name( 'display_member_name' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'display_member_name' ) ); ?>">
				<option value="show"
					...
					if ( 'show' === $display_member_name ) :
						?>
					selected="selected"<?php endif; ?>><?php esc_html_e( 'Show', 'buddypress' ); ?></option>
				<option value="hide"
					<?php
					if ( 'hide' === $display_member_name ) :
						?>
					selected="selected"<?php endif; ?>><?php esc_html_e( 'Hide', 'buddypress' ); ?></option>
			</select>
		</p>
		<?php
	}
