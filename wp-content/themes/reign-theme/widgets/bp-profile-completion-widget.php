...
class BP_Reign_Profile_Completion_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => __( 'Show Logged in user Profile Completion Progress.', 'reign' ),
		);

		/* Set up the widget. */
		parent::__construct(
			false,
			__( '(BuddyPress) Profile Completion', 'reign' ),
			$widget_ops
		);
	}
	...
	function widget( $args, $instance ) {
		 ...
		 */
		if ( ! empty( $instance['profile_hide_widget'] ) && 100 === (int) $user_progress['completion_percentage'] ) {
			return;
		}
					...
					<div class="wb-bp-user-avatar-wrap">
						<div class="wb-bp-user-avatar">
							...
							if ( ( $current_user instanceof WP_User ) ) {
								...
								echo '<a class="wb-bp-user-link" href="' . $user_link . '">'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								?>
								<?php
						...
						<div class="wb-bp-progress-label">
							<span class="wb-bp-completion"><?php echo $user_progress['completion_percentage'] . '%'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
							<span><?php echo esc_html__( 'Complete', 'reign' ); ?></span>
						</div>
					</div>
				</div>
			...
			<div class="wb-bp-detailed-progress-container">
				<ul class="wb-bp-detailed-progress">
					...
					foreach ( $user_progress['groups'] as $single_section_details ) :
						...
						<?php
						echo ( $single_section_details['is_group_completed'] ) ? esc_attr( 'completed ' ) : esc_attr( 'incomplete ' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	function get_user_profile_progress( $group_ids, $photo_types ) {
		...
		$progress_details['groups'][] = array(
			'group_name'             => __( 'Profile Fields', 'reign' ),
	...
	function form( $instance ) {
		...
		if ( ! $is_profile_photo_disabled ) {
			$photos_enabled_arr['profile_photo'] = __( 'Profile Photo', 'reign' );
		}
		if ( ! $is_cover_photo_disabled ) {
			$photos_enabled_arr['cover_photo'] = __( 'Cover Photo', 'reign' );
		}
		...
		<p>
			...
			<label><?php esc_html_e( 'Profile field sets:', 'reign' ); ?></label>
		...
		<?php if ( ! empty( $photos_enabled_arr ) ) : ?>
			<p>
				<label><?php esc_html_e( 'Profile photos:', 'reign' ); ?></label>
		...
		<p>
			<label>
				...
				<?php echo __( 'Hide widget once progress hits 100%', 'reign' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			</label>

		</p>
		<p>
			<small>
				<?php esc_html_e( 'Note: This widget is only displayed if a member is logged in.', 'reign' ); ?>
			</small>
		</p>
...
add_action(
	...
	function () {
		if ( class_exists( 'BuddyPress' ) ) {
			register_widget( 'BP_Reign_Profile_Completion_Widget' );
		}
	}
);
