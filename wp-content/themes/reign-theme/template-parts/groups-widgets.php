...
<div class="widget widget-groups-by rg-custom-mbl-menu">
	<h4 class="widget-title">
		<span><?php esc_html_e( 'Groups', 'buddypress' ); ?></span>
	...
	<div class="item-list-tabs" aria-label="<?php esc_attr_e( 'Groups directory main navigation', 'buddypress' ); ?>">
		<ul>
			<?php if ( function_exists( 'buddypress' ) && version_compare( buddypress()->version, '12.0', '>=' ) ) : ?>
				<li class="selected" id="groups-all"><a href="<?php bp_groups_directory_url(); ?>"><?php printf( __( 'All Groups %s', 'buddypress' ), '<span>' . bp_get_total_group_count() . '</span>' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></a></li>
			<?php else : ?>
				<li class="selected" id="groups-all"><a href="<?php bp_groups_directory_permalink(); ?>"><?php printf( __( 'All Groups %s', 'buddypress' ), '<span>' . bp_get_total_group_count() . '</span>' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></a></li>
			<?php endif; ?>

			<?php if ( is_user_logged_in() && bp_get_total_group_count_for_user( bp_loggedin_user_id() ) ) : ?>
				<li id="groups-personal"><a href="<?php echo bp_loggedin_user_domain() . bp_get_groups_slug() . '/my-groups/'; ?>"><?php printf( __( 'My Groups %s', 'buddypress' ), '<span>' . bp_get_total_group_count_for_user( bp_loggedin_user_id() ) . '</span>' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></a></li>
			<?php endif; ?>
...
if ( class_exists( 'Bp_Add_Group_Types' ) ) {
	...
	<div class="widget widget-groups-groupby rg-custom-mbl-menu">
		<h4 class="widget-title">
			<span><?php esc_html_e( 'Group Types', 'buddypress' ); ?></span>
			<span class="custom-icon ico-plus fa fa-plus-circle"></span>
			<span class="custom-icon ico-minus fa fa-minus-circle"></span>
		</h4>
		<?php echo $group_select_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		<div class="item-list-tabs" id="subnav" aria-label="<?php esc_attr_e( 'Groups directory secondary navigation', 'buddypress' ); ?>" role="navigation">
			<ul class="wb-group-type-filters-wrap">
				<?php echo $group_select_li_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			</ul>
		</div>
	</div>
	<?php
