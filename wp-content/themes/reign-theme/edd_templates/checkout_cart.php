...
>
	<thead>
		<tr class="edd_cart_header_row">
			...
			<th class="edd_cart_item_name"><?php esc_html_e( 'Item Name', 'easy-digital-downloads' ); ?></th>
			<th class="edd_cart_item_price"><?php esc_html_e( 'Item Price', 'easy-digital-downloads' ); ?></th>
			<th class="edd_cart_actions"><?php esc_html_e( 'Actions', 'easy-digital-downloads' ); ?></th>
	...
	<tbody>
			...
			<?php foreach ( $cart_items as $key => $item ) : ?>
				<tr class="edd_cart_item" id="edd_cart_item_<?php echo esc_attr( $key ) . '_' . esc_attr( $item['id'] ); ?>" data-download-id="<?php echo esc_attr( $item['id'] ); ?>">
					...
					<td class="edd_cart_item_price">
						<?php
						echo edd_cart_item_price( $item['id'], $item['options'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					...
					<td class="edd_cart_actions">
						...
						<a class="edd_cart_remove_item_btn" href="<?php echo esc_url( wp_nonce_url( edd_remove_item_url( $key ), 'edd-remove-from-cart-' . sanitize_key( $key ), 'edd_remove_from_cart_nonce' ) ); ?>"><i class="fa fa-times"></i><?php // _e( 'Remove', 'easy-digital-downloads' ); ?></a>
					</td>
		...
		<?php if ( edd_cart_has_fees() ) : ?>
			<?php foreach ( edd_get_cart_fees() as $fee_id => $fee ) : ?>
				<tr class="edd_cart_fee" id="edd_cart_fee_<?php echo $fee_id; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>">
					...
					<td>
						<?php if ( ! empty( $fee['type'] ) && 'item' == $fee['type'] ) : ?>
							<a href="<?php echo esc_url( edd_remove_cart_fee_url( $fee_id ) ); ?>"><i class="fa fa-times"></i><?php // _e( 'Remove', 'easy-digital-downloads' ); ?></a>
						<?php endif; ?>
	...
	<tfoot>
		...
		<?php if ( edd_use_taxes() && ! edd_prices_include_tax() ) : ?>
			...
			>
				...
				<th colspan="<?php echo absint( edd_checkout_cart_columns() ); ?>" class="edd_cart_subtotal">
					<?php esc_html_e( 'Subtotal', 'easy-digital-downloads' ); ?>:&nbsp;<span class="edd_cart_subtotal_amount"><?php echo edd_cart_subtotal(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
				</th>
		...
		<?php if ( edd_use_taxes() ) : ?>
			...
			>
				...
				<th colspan="<?php echo esc_attr( edd_checkout_cart_columns() ); ?>" class="edd_cart_tax">
					<?php esc_html_e( 'Tax', 'easy-digital-downloads' ); ?>:&nbsp;<span class="edd_cart_tax_amount" data-tax="<?php echo edd_get_cart_tax( false ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>"><?php echo esc_html( edd_cart_tax() ); ?></span>
				</th>
		...
		<tr class="edd_cart_footer_row">
			...
			<th colspan="<?php echo esc_attr( edd_checkout_cart_columns() ); ?>" class="edd_cart_total"><?php esc_html_e( 'Total', 'easy-digital-downloads' ); ?>: <span class="edd_cart_amount" data-subtotal="<?php echo esc_attr( edd_get_cart_subtotal() ); ?>" data-total="<?php echo esc_attr( edd_get_cart_total() ); ?>"><?php edd_cart_total(); // Escaped ?></span></th>
